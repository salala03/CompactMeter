...
std::vector<std::pair<int, int>> GetGPUMemory() {
    ...
    DWORD instanceListSize = 0;
    DWORD dwFlags = 0;
    const auto COUNTER_OBJECT = TEXT("GPU Process Memory");
    PDH_STATUS status = ERROR_SUCCESS;
    status = PdhEnumObjectItems(nullptr, nullptr, COUNTER_OBJECT, nullptr,
        &counterListSize, nullptr, &instanceListSize,
        PERF_DETAIL_WIZARD, dwFlags);
    ...
    for (TCHAR* pTemp = instanceList.data(); *pTemp != 0;
        pTemp += _tcslen(pTemp) + 1) {
        if (::_tcsstr(pTemp, TEXT("#")) != NULL) {
            continue;
        }

        if (::_tcsstr(pTemp, TEXT("phys_0")) == NULL) {
            continue;
        }


        TCHAR buffer[1024];
        ::StringCchCopy(buffer, 1024, TEXT("\\GPU Process Memory("));
        ::StringCchCat(buffer, 1024, pTemp);
        ::StringCchCat(buffer, 1024, TEXT(")\\Dedicated Usage"));
...
double GetGPUMemoryTotal() {
    int64_t total = 0;
    std::vector<std::pair<int, int>> list = GetGPUMemory();
    for (const std::pair<int, int>& v : list) {
        if (v.second > 0) {
            total += v.second;
        }
